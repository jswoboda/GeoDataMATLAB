...
for k =1:length(omtikeep)
    ...
    for irt =1:length(risrtimes)
        ...
        drawnow
        % Link to deal be able to move the axes if you change the size.
        linkaxes([hax3,hax3b],'xy');
        % Plot the contours using the new axis
        [contimg, h] = contourGD(risrGD,'z',400,'value','key','ne','Fig',hfig,'axh',hax3b,'title'...
            ,'N_e at $thm','time',curirt,'bounds',[5e9,5e11]);
        
        hcb3 = colorbar();
        
        set(hax3,'pos',get(hax3b,'pos'))
